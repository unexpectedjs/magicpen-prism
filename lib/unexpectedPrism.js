...
module.exports = function unexpectedPrism(expect) {
    Object.keys(colorByTokenType).forEach(function (tokenType) {
        expect.output.addStyle('prism:' + tokenType, function (content) {
            this.text(content, colorByTokenType[tokenType]);
        });
    });

//console.log(expect.output.styles);

    expect.output.addStyle('code', function (sourceText, language) {
